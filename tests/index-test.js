...
describe('JsonPathEditor', () => {
    ...
    it('should overrides editor position when a position is given', () => {
        const wrapper = mount(<JsonPathEditor editorPosition={{x: 20, y: 20}} />);
        expect(wrapper.state().editorPosition).toEqual({x: 20, y: 20});
    });

    it('should updateEditorPosition do nothing when input ref is not defined', () => {
        const wrapper = mount(<JsonPathEditor />);
        const initialPosition = wrapper.state().editorPosition;
        wrapper.instance().inputRef = undefined;
        wrapper.instance().updateEditorPosition();
        expect(wrapper.state().editorPosition).toEqual(initialPosition);
    });
});
